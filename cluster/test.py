...
data_size_list = [1.25 * 1e2, 1.25 * 1e3, 1.25 * 1e4]
    ...
    for cluster_size in cluster_size_list:
        for dim in dim_list:
            try:
                ...
                clf = GridSearchCV(estimator=sqrt_lasso_gmm(n_iter=100), param_grid=param, cv=5, n_jobs=-1,
                                   scoring=bic_scorer)
                clf.fit(X_train, y_train)

                params_GMM = {"n_components": range(2, max_clusters + 1)}
                clf_gmm = GridSearchCV(GMM(covariance_type='full'), param_grid=params_GMM, cv=5, n_jobs=-1,
                                       scoring=bic_scorer)
                clf_gmm.fit(X_train)
