...
class lassoEM(BaseEstimator):
    ...
    # TODO: If a cvxpy trace operator becomes available, use it!
        obj = cvx.Minimize(-1/2*cvx.log_det(S) -1/2*sum([(S*np.array(snk))[i, i] for i in range(n)]))
...
if __name__ == '__main__':
    ...
    weigths = [0.2, 0.5, 0.3]
    centers = [[0, 0], [0, 3], [5, 0]]
    vars = [[[3, 1],[1, 1]], [[3, 1],[1, 1]], [[4,0],[0,2]]]

    X = gaussM(weigths,centers,vars, 1000)
    lasso = lassoEM(n_components=3, n_iter=10, lambd= 0.10)
    lasso.fit(X)

    print "Lasso ==============="
    print "means "
    print lasso.centers_
    print "covars "
    print lasso.omega_
    print "weights "
    print lasso.pi_

    cl = GMM(n_components=3, n_iter = 10,  covariance_type='diag')
    cl.fit(X)
    print "GMM ================"
    print "weights "
    print cl.weights_
